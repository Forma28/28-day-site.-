...
3. **Заполните Script Properties.** В интерфейсе Apps Script перейдите в `Project Settings → Script properties` и добавьте ключи:
   ...
   - При необходимости: `BOT_USERNAME`, `CHANNEL_LINK`, `CHAT_LINK`, `SUPPORT_LINK`, реквизиты оплаты и `OPENAI_API_KEY`.
...
7. **Проверьте команды.** В Telegram введите `/start`, затем протестируйте `/plan`, `/nutrition`, `/pro`, чек-ин по кнопкам. Заполните лист `plans` собственными тренировками.
